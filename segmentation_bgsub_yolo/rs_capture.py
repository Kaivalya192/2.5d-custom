...
def capture_background(
    ...
    while captured < num_frames:
        ...
        disp = frame.copy()
        cv2.putText(disp, f"Background capture: {captured}/{num_frames}", (20, 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2, cv2.LINE_AA)
        cv2.imshow("RealSense Background Capture", disp)
...
def capture_objects(
    ...
    while saved < num_frames:
        ...
        disp = frame.copy()
        cv2.putText(disp, f"Object capture: {saved}/{num_frames}", (20, 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow("RealSense Object Capture", disp)
        key = cv2.waitKey(1) & 0xFF
        if key in (ord("q"), 27):
            break
        if key == ord("s"):
            saved += 1
            path = os.path.join(out_dir, f"object_{saved:06d}.png")
            cv2.imwrite(path, frame)
            print(f"[Saved] {path}")

    print(f"[OK] Saved {saved} object images to {out_dir}")


def parse_args():
    parser = argparse.ArgumentParser(description="RealSense capture for segmentation dataset generation.")
    sub = parser.add_subparsers(dest="mode", required=True)

    for mode in ("background", "objects"):
        sp = sub.add_parser(mode)
        sp.add_argument("--out_dir", type=str, required=True)
        sp.add_argument("--num_frames", type=int, default=30 if mode == "background" else 300)
